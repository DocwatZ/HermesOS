...
services:
  hermesos:
    ...
    volumes:
      - /path/to/data:/config
    ports:
      - 3000:3000   # Selkies desktop (HTTP)
      - 3001:3001   # Selkies desktop (HTTPS)
      - 3002:3002   # HermelinChat web UI
    shm_size: "1gb"
    security_opt:
      - seccomp:unconfined
...
| HermelinChat | `http://YOUR-HOST:3002/` | Hermes Agent web UI. Set password in `/config/.hermelin.env`. |
...
HermelinChat starts automatically as a background service and is available at `http://YOUR-HOST:3002/`.
...
| Parameter | Function |
| :----: | --- |
| `-p 3000:3000` | Selkies desktop (HTTP) |
| `-p 3001:3001` | KDE Plasma desktop (HTTPS) |
| `-p 3002:3002` | HermelinChat web UI |
| `-e PUID=1000` | User ID |
...
### "Welcome to nginx!" on the desktop port

nginx is running but the Selkies streaming service has not started yet. Wait 60–90 seconds after first boot, then refresh. If the problem persists, try disabling Wayland (`-e PIXELFLUX_WAYLAND=false`) and restarting. If you are using a non-default HTTPS port (e.g. `3311`), set `-e CUSTOM_HTTPS_PORT=3311` **and** map `-p 3311:3311`; do not map it to 3001.

### HermelinChat: `PermissionError: Permission denied: '/config/.hermes/cron'`

The `hermes` CLI cannot create its config subdirectory. This is fixed in the current image (the run script now chowns `/config/.hermes` to `abc` on every start). On an older image, run:

```bash
docker exec -it HermesOS chown -R abc:abc /config/.hermes
```

Then restart the container. Ensure you have added at least one LLM provider key to `/config/.hermes/.env` before starting a hermelinChat session.

---

## Versions

* **27.05.26:** — Fix `PermissionError: /config/.hermes/cron` by chowning the hermes home dir to `abc` in the hermelinChat service script. Correct port documentation (HermelinChat is on `3002`, not `3000`).
